...
import RsolarLoginPasswordRender from './RsolarLoginPassword';

const RsolarLoginPasswordLogic = (props:any) => {
  ...
  const submitOtps = ()=>{
    console.log('sachin')
    postUnAuthReq('/auth/verify-otp-login-solar', {mobile: props.route.params.mobile,otp:loginForm.password}).then((res: any) => {
        let result = res.data
  ...
  const apiCallOnSubmitForm = async () => {
    ...
    await postUnAuthReq('/auth/solar-login', formData)
      .then((res: any) => {
        let result = res.data;
        console.log('response data from login', result.data);
        ...
        if (result?.data?.token) {
          navigation.navigate('signIn', {screen: 'rsolar-home'});
        }
      })
      .catch(error => {
        console.log(
...
export default RsolarLoginPasswordLogic;
